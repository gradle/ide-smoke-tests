...
class IdeaFrame(remoteRobot: RemoteRobot, remoteComponent: RemoteComponent) : CommonContainerFixture(remoteRobot, remoteComponent) {
    ...
    val projectViewTree
        get() = findAll<ContainerFixture>(byXpath("ProjectViewTree", "//div[@class='ProjectViewTree']"))

    val projectToggle
        get() = find<ContainerFixture>(byXpath("Project", "//div[contains(@text.key, 'title.project')]"))
